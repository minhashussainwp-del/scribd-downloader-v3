      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <main className="lg:col-span-8 bg-white rounded-3xl p-6 sm:p-10 border border-slate-200 shadow-2xs space-y-6">
            ...
            {/* FAQ accordion (structured faqs field) */}
            {page.faqs && page.faqs.length > 0 && (
              <div className="pt-4 space-y-3">
                <h2 className="text-lg font-extrabold text-slate-900">Frequently Asked Questions</h2>
                <div className="space-y-2.5">
                  {page.faqs.map((f, fIdx) => (
                    <details
                      key={fIdx}
                      className="bg-slate-50 border border-slate-200/80 rounded-xl px-5 py-4 group"
                    >
                      <summary className="text-sm font-bold text-slate-900 cursor-pointer list-none flex items-center justify-between gap-3">
