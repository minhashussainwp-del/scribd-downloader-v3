...
export default function App() {
  ...
  // Get current how-it-works content based on selected language
  const currentHowItWorksContent =
    pageContents.find(
      (c) =>
        (c.pageKey === "how-it-works" && c.language === currentLang) ||
        c.id === `how-it-works-${currentLang}`
    ) || pageContents.find((c) => c.id === "how-it-works");
              ...
              <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-12 pb-6">
                <div className="text-center max-w-2xl mx-auto space-y-3">
                  <span className="text-xs font-bold uppercase tracking-wider text-purple-600 bg-purple-50 px-3 py-1 rounded-full border border-purple-100">
                    {t("hiw.badge", currentLang)}
                  </span>
                  <h1 className="text-3xl sm:text-4xl font-extrabold text-slate-900">
                    {currentHowItWorksContent?.title || t("hiw.title", currentLang)}
                  </h1>
                  <p className="text-sm sm:text-base text-slate-600">
                    {currentHowItWorksContent?.subtitle || t("hiw.sub", currentLang)}
                  </p>
                </div>
              </div>
              <HomeContent
                ...
                onSelectPost={handleSelectPost}
                currentLang={currentLang}
                pageContent={currentHowItWorksContent}
              />
            </div>
          )}
